                       ...
                       const gmx_output_env_t *oenv)
{
    ...
    if (DD)
    {
        fprintf(out, "# Displacment gathered over %g %s with %d restarts\n",
                    ...
                    real dt, real beginfit, real endfit, const gmx_output_env_t *oenv)
{
    ...
    fprintf(stderr, "\nSelect a group to calculate mean displacement for:\n");
...
int gmx_displ(int argc, char *argv[])
{
    const char        *desc[] = {
        "[THISMODULE] computes the mean displacement (MSD) of atoms from",
